...
import { CheckCircle2, Eye, RotateCw, Sparkles } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function SalesIntakeForm({
  ...
  // Show confidence when auto-filled data exists (helps user see AI accuracy)
  const showConfidence = isAutoFilled && fieldConfidence !== null
  ...
  // Editable form (always editable when data exists)
  return (
    <div className={`mt-4 p-4 border rounded-lg bg-white ${isConfirmed ? 'border-[#C8E4BB]' : 'border-[#407B9D]/30'}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          {isConfirmed && (
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[#C8E4BB]/20 text-[#5A8A4A] border border-[#C8E4BB]">
        ...
        {/* View Details button for quick read-only view */}
        {localData && (
          <Button
            variant="outline"
            size="sm"
      ...
      <p className="text-sm text-muted-foreground mb-4" style={{ fontFamily: "var(--font-body)" }}>
        {isConfirmed
          ? "This intake has been confirmed. You can still make changes and save them."
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t">
            ...
            >
              ...
              {isConfirmed ? 'Save Changes' : 'Confirm Intake'}
            </Button>
          </div>
        </div>
      )}
