...
export function ManageTeamModal({
  ...
  useEffect(() => {
    if (isOpen) {
      setRecipients(getManagedRecipients())
      // Reset states
      setIsAdding(false)
      setEditingId(null)
      setDeletingId(null)
  ...
  const handleAddRecipient = () => {
    ...
    if (recipientEmailExists(newEmail)) {
      setNewError("This email already exists in the team")
      return
    }

    addManagedRecipient(newName.trim(), newEmail.trim().toLowerCase())
    setRecipients(getManagedRecipients())
    setNewName("")
    setNewEmail("")
    setIsAdding(false)
  ...
  const handleSaveEdit = () => {
    ...
    if (recipientEmailExists(editEmail, editingId)) {
      setEditError("This email already exists in the team")
      return
    }

    updateManagedRecipient(editingId, {
      name: editName.trim(),
