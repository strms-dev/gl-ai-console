...
import { ExternalLink, AlertCircle, RefreshCw, Link2 } from "lucide-react"
import { AppHelpModal, HelpButton } from "@/components/shared/app-help-modal"

const QBO_MCP_URL = "https://qbo-mcp.vercel.app/"
const IFRAME_LOAD_TIMEOUT = 5000 // 5 seconds

export default function QboMcpPage() {
  ...
  const openInNewTab = () => {
    window.open(QBO_MCP_URL, "_blank", "noopener,noreferrer")
  }
  ...
  const retryIframe = () => {
    ...
    if (iframeRef.current) {
      iframeRef.current.src = QBO_MCP_URL
    }
  }
    ...
    <div className="flex flex-col h-screen bg-[#FAF9F9]">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-white">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#407B9D]/10 flex items-center justify-center">
            <Link2 className="w-5 h-5 text-[#407B9D]" />
          </div>
          <div>
            <h1
            ...
            >
              QuickBooks Online MCP Connection Manager
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 relative">
            ...
            <Card className="max-w-md mx-4 bg-white border-none shadow-lg">
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle
                  ...
                  style={{ fontFamily: "var(--font-heading)" }}
                >
                  Unable to Embed QBO MCP
                </CardTitle>
                <CardDescription
                  className="text-sm"
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  QBO MCP may have security settings that prevent embedding. You can still access it in a new tab.
                </CardDescription>
              </CardHeader>
        ...
        <iframe
          ...
          title="QBO MCP - QuickBooks Online MCP Connection Manager"
      ...
      <AppHelpModal
        appName="QBO MCP"
        appDescription="Connect and manage QuickBooks Online accounts for MCP integrations. This tool allows authorized users to link QBO accounts to the MCP system."
        isOpen={showHelp}
